...
export const CalendarPage = ({ activities, onDelete, onSelectActivity }) => {
    ...
    const [plannedActivities, setPlannedActivities] = useState(() => {
        const saved = localStorage.getItem('forma_planned_workouts');
        return saved ? JSON.parse(saved) : [];
    });

    useEffect(() => {
        localStorage.setItem('forma_planned_workouts', JSON.stringify(plannedActivities));
    }, [plannedActivities]);

    const [isModalOpen, setIsModalOpen] = useState(false);
    const [selectedDateForPlan, setSelectedDateForPlan] = useState(null);
    const [newPlan, setNewPlan] = useState({ type: 'Run', name: '', tss: 50, duration: 60 });
    ...
    const handleSavePlan = () => {
        ...
        setPlannedActivities(prev => [...prev, newAct]);
        setIsModalOpen(false);
        setNewPlan({ type: 'Run', name: '', tss: 50, duration: 60 });
    };
    ...
    const activitiesByDate = useMemo(() => {
        ...
        if (plannedActivities) {
            plannedActivities.forEach(pAct => {
        ...
        <>
                                  ...
                                  `}>
                                                ...
                                                <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-1 w-full pr-0.5">
                                                    {acts.map((act, i) => (
                                                        <div
                                                            key={i}
                                                            onClick={() => act.isPlanned ? null : (onSelectActivity && onSelectActivity(act))}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-slate-900/50 dark:bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl max-w-sm w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950/50">
                            ...
                            <button onClick={() => setIsModalOpen(false)} className="p-1.5 text-slate-400 hover:bg-slate-200 dark:hover:bg-zinc-800 rounded-lg transition-colors"><X size={16} /></button>
                        </div>

                        <div className="p-4 space-y-4">
                            <div>
                                <label className="block text-[10px] font-bold text-slate-600 dark:text-zinc-400 uppercase tracking-widest mb-1.5">Deporte</label>
                                <select
                                    value={newPlan.type}
                                    onChange={e => setNewPlan({ ...newPlan, type: e.target.value })}
                                    className="w-full bg-slate-100 dark:bg-zinc-950 border-none text-sm font-bold text-slate-800 dark:text-zinc-200 rounded-lg p-3 outline-none ring-1 ring-slate-200 dark:ring-zinc-800 focus:ring-2 focus:ring-blue-500"
                                >
                                    <option value="Run">Carrera (Run)</option>
                                    <option value="Ride">Ciclismo (Ride)</option>
                                    <option value="Swim">Natación (Swim)</option>
                                    <option value="WeightTraining">Fuerza (Gym)</option>
                                    <option value="Workout">Otro</option>
                                </select>
                            </div>

                            <div>
                                <label className="block text-[10px] font-bold text-slate-600 dark:text-zinc-400 uppercase tracking-widest mb-1.5">Título (Opcional)</label>
                                <input
                                    type="text"
                                    placeholder="Ej: Series 4x5 min"
                                    value={newPlan.name}
                                    onChange={e => setNewPlan({ ...newPlan, name: e.target.value })}
                                    className="w-full bg-slate-100 dark:bg-zinc-950 border-none text-sm font-bold text-slate-800 dark:text-zinc-200 rounded-lg p-3 outline-none ring-1 ring-slate-200 dark:ring-zinc-800 focus:ring-2 focus:ring-blue-500 placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:placeholder:opacity-0 transition-all text-left"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="flex items-center gap-1 text-[10px] font-bold text-slate-600 dark:text-zinc-400 uppercase tracking-widest mb-1.5"><Zap size={10} /> TSS Esperado</label>
                                    <input
                                        type="number"
                                        min="0"
                                        value={newPlan.tss}
                                        onChange={e => setNewPlan({ ...newPlan, tss: e.target.value })}
                                        className="w-full bg-slate-100 dark:bg-zinc-950 border-none text-sm font-bold text-slate-800 dark:text-zinc-200 rounded-lg p-3 outline-none ring-1 ring-slate-200 dark:ring-zinc-800 focus:ring-2 focus:ring-blue-500"
                                    />
                                </div>
                                <div>
                                    <label className="flex items-center gap-1 text-[10px] font-bold text-slate-600 dark:text-zinc-400 uppercase tracking-widest mb-1.5"><Clock size={10} /> Duración (min)</label>
                                    <input
                                        type="number"
                                        min="0"
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-zinc-950/50 border-t border-slate-200 dark:border-zinc-800 flex justify-end gap-2">
                            ...
                            <button onClick={handleSavePlan} className="px-4 py-2 rounded-lg text-xs font-black text-white bg-blue-600 hover:bg-blue-500 uppercase tracking-wider transition-colors shadow-lg shadow-blue-500/20">Añadir al Plan</button>
                        </div>
                    </div>
                </div>
            )}
