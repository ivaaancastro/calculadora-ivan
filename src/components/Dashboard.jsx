...
import { Loader2, FileText, Plus, Database, Settings, RefreshCw, Activity } from "lucide-react";
...
const Dashboard = () => {
  const {
    activities, loading, uploading, uploadStatus, timeRange, settings, setTimeRange,
    handleClearDb, processFile, fetchActivities, fetchProfile, analyzeHistory,
    filteredData, currentMetrics, chartData, distribution, summary,
    isStravaConnected, handleStravaSync, deleteActivity, fetchActivityStreams,
    isDeepSyncing, deepSyncProgress, handleDeepSync, updateProfile,
  } = useActivities();
  ...
  if (loading && !uploading) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-zinc-950 gap-4 transition-colors">
        ...
        <p className="text-[10px] text-slate-500 dark:text-zinc-400 font-bold uppercase tracking-widest animate-pulse">Cargando métricas...</p>
      </div>
    );
  }
        ...
        activities.length === 0 ? (
          <div className="text-center py-20 px-4">
            <div className="bg-white dark:bg-zinc-900 rounded-lg border border-slate-200 dark:border-zinc-800 p-8 max-w-md mx-auto">
              <Database size={32} className="mx-auto text-slate-400 dark:text-zinc-600 mb-4" />
              <h2 className="text-sm font-bold uppercase tracking-widest text-slate-800 dark:text-zinc-100 mb-2">Base de datos vacía</h2>
              <p className="text-[10px] text-slate-500 dark:text-zinc-500 uppercase tracking-wider mb-6">Conecta Strava para iniciar el análisis.</p>
            </div>
          </div>
        ) : (
          ...
          <>
            ...
            {/* VISTA: OVERVIEW */}
            <div className={activeTab === "overview" ? "block space-y-4 animate-in fade-in duration-300" : "hidden"}>
              
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-2">
                <div>
                  <h2 className="text-xl font-black text-slate-900 dark:text-zinc-100 tracking-tight uppercase">Panel de Rendimiento</h2>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-zinc-500">Métricas y Adaptaciones Fisiológicas</p>
                </div>
                
                {/* SELECTOR DE TIEMPO */}
                <div className="flex border border-slate-200 dark:border-zinc-700 rounded overflow-hidden self-start sm:self-auto">
                  {[
                    { id: "7d", label: "7D" }, { id: "30d", label: "30D" }, { id: "90d", label: "3M" },
                    { id: "1y", label: "1A" }, { id: "all", label: "Todo" },
                  ].map((t) => (
                    <button
                      key={t.id}
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-zinc-800 mt-4">
                <AdvancedAnalytics activities={filteredData} settings={settings} />
              </div>
            </div>
